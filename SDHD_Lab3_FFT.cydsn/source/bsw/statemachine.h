...
*/
 enum eStateMachineEnum{
  IDLE_STATE,           /**< IDLE State. */
  SAMPLING_STATE,       /**< Sampling State. */
  UART_TRANSFER_STATE   /**< Uart Transfer State. */
} ;
typedef enum eStateMachineEnum myStateMachine_t;
...
/**
 * An interrupt function that stores 1 in toggle variable if PushButton is pressed.
 */
CY_ISR_PROTO(isr_PushButton_Enable_Interrupt);
...
/**
 * Function that samples voltage divider data through ADC and transfer it to MATLAB using UART. 
 * @param: SamplingArray - that stores the 1024 ADC samples.
 * @return: None
 */
